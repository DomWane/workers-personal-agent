...
written the way I actually ask them. Czech stemming closes about a fifth of that gap at no
inference cost. Static word vectors, unweighted and mean-pooled, are not competitive here — §7 on
how narrow that result is. Neither hybrid fusion nor a cross-encoder reranker improved on plain
dense retrieval — §4. Five measurements were wrong before
they were right; §6 lists them, because they are the transferable part.

## 1. Data

673 exchanges from my own Claude Code transcripts, 28 working days (2026-06-13 to 2026-07-30),
117k words, median 1177 characters. One human turn plus the assistant reply is one chunk — the
unit the agent actually retrieves. About three quarters are engineering work, the rest is building
the agent itself.

The corpus is the author's own working sessions and stays on the machine that produced it: it is
not in the repository, not shared, and nothing in it is quoted here. Questions and findings are
described rather than reproduced. The harness reproduces; the result does not.
...
the corpus was searched. Two rules are enforced by the labelling tool rather than by intention:
...
Stemming on the bulk set is the one comparison that does not clear zero and is reported as
unresolved. The split is explainable: median share of query words present in the relevant chunks
is 0.22 on bulk against 0.29 on hard, so on the bulk side there is less lexical overlap for
...
hard). nDCG and MRR lead the table for this reason. **recall@10 is a lower bound** — judging
covered the union of the top-3 of all seven pooled systems — the five above plus the fusion and
the reranker of §4 — so positions 4–10 hold unjudged chunks.
...
against single-chunk ground truth and understates every retriever — uniformly, so the comparisons
hold, but the absolute bulk numbers are floors rather than estimates.
...
judging the union of every system's top-k, blind to which system returned what — raised the hard
set to 2.73 relevant chunks per query over **394 judgments**, of which 26 were accepted. The low
acceptance rate is itself a finding: on this corpus a specific question is usually answered once.

Pooling cannot fix its own blind spot. A relevant chunk no retriever ranks in its top-3 is never
offered for judging. One query's episode had four decision points and only one surfaced; the other
three were added by hand, which lowers every retriever's recall equally.
...
candidate window. Both were built and measured.
...
Weighting the fusion towards the stronger retriever monotonically reduces the damage, converging on
plain dense. There is no weight at which fusion helps. The reranker moved the relevant chunk **up
for 6 bulk queries and down for 28** — the failure a reranker exists to fix, running backwards.
...
entirely on 4. But those chunks sit at ranks 10–20 of a list whose first ten are wrong, and RRF at
k=60 cannot lift a rank-12 entry past a rank-2 one from a retriever 0.13 nDCG stronger. Fusion
assumes comparable inputs.

**The reranker's failure is not truncation.** Re-run at 600, 1600 and 3000 characters per
candidate, the scores are identical within noise (hard 0.338 / 0.335 / 0.336), which also indicates
...
added 67 assessments (5 accepted, a 7% rate against 6% in the earlier batch — the same standard).
...
**Unranked search during labelling** (§2) — the single decision keeping the hard set from
...
**Recording bias instead of arguing it away.** Where a hand-edited query or a search-located
...
stemmed BM25 0.47 against 0.24 — and *lower* for bge-m3 (0.48 against 0.53): the edits added
words, which helps only lexical matching. Seven of 78 queries are affected, so these are an
indication, not a measurement.

## 6. Five silent failures, and what caught each

Each produced a plausible number. None failed loudly, which is the point of listing them.
...
me choosing the queries — perfect lexical matches that answer nothing, inflating the lexical
retrievers and
polluting the judging pool. The labelling tool's similar-chunk suggestion then offered them as
also-relevant and I accepted three.

Fixed with a cutoff date, not a content rule: excluding chunks containing a labelled query would
let the label set decide the corpus. Cost 86 chunks and one label. The BM25-over-substring gap,
which had looked established, dissolved to [−0.031, 0.115] on the clean corpus. It re-established
itself later on a larger, properly judged set — but the significance at that moment had been
partly contamination.

**2. A hardcoded cap made an experiment measure nothing.** Testing whether longer chunks retrieve
better, caps of 2400 and 3600 characters produced byte-identical numbers: the embedding step
truncated its input at 1600. It read as a clean null result, which is exactly what an unnoticed
no-op looks like. The same investigation surfaced a vector cache that invalidated on corpus text
but not on embedding parameters, so raising the limit silently reused stale vectors.
...
BM25 was +0.228 [0.042, 0.413] before and +0.200 [0.024, 0.379] after — the point estimate the
table in §3 still reports, on an interval later judging narrowed to [0.070, 0.326]. A single
lenient pass could not have shown the result was not an artifact of generous judging.
...
truth the others built — it loses quietly, nothing fails. Including it added 31 candidates only it
surfaced. I then repeated the mistake with fastText and caught it while re-reading my own
...
- **The static-vector row is one configuration, not a verdict on static embeddings.** fastText was
  ...
  an edge runtime and the reason it was the baseline — not the strongest way to use static vectors.
  Term weighting, SIF-style common-component removal and aligned cross-lingual vectors are all
  untested here, and the first two are the standard answers to exactly this setting. What the row
  supports is that naive pooling does not survive the move to paragraph-length chunks; the ceiling
  for the method is not what was measured.
