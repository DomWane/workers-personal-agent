...
tokens on average, and over 90% less in best cases."* The release post measures it carefully, with
twelve out-of-domain benchmarks, five seeds and per-question matched deltas, and lands at −45.8%
out of domain, −57.7% in domain.
...
- **What was the bottleneck?** Retrieval rather than reasoning. On 40% of questions the answer was
...
Each model receives the same top-5 chunks from `bge-m3` and answers from those alone, so the input
is byte-identical.
...
recorded, so which host answered any given question is unrecoverable. The throughput shows that the
routing varied: the base model's runs span 24 to 97 tok/s against 64 to 88 for the fine-tune, whose
host disclosed no quantisation at all.
...
Both problems went away once the two published FP8 checkpoints were deployed to identical
hardware, for about $6 of GPU time.
...
The primary metric is therefore trace length in **characters**, a stand-in for tokens that the
broken field forced. It is only valid if both models spend characters at the same rate, so that was
checked: across all 180 completions the median is **3.48 characters per completion token for the
base model and 3.46 for the fine-tune**, 0.5% apart. The two share a tokenizer and both write Czech
prose, and the ratio confirms it. A character ratio between them is a token ratio. The
independently measured completion-token saving, 48.8% against 50.8%, is the same statement arrived
at without the substitution.
...
saving would have measured zero. One smoke call before the paid run caught it; the run itself
would not have.
...
reasoning. That case is handled explicitly; nothing truncated in the event.
...
domain. The interval here spans both, on a task outside every benchmark in the suite. The
...
median of 45% (range across seeds; base 43.1%, tuned 45.6%, so the fine-tune is shorter without
being steadier). This is why three seeds were run and why no single question's number is quoted
anywhere in this document. A one-pass design, which is what this pre-registration originally
called for, would have reported a figure substantially made of sampling.
...
discussed with chunks quoted. Prior knowledge of a question favours neither model, so the
comparison stands; it weakens the set's second use, below.
...
same retrieval errors by design, because that is the setting the agent runs in, but it means 40% of
the sample is measuring refusal.
...
- The two FP8 quantisations share format and block size but not their exclusion lists. The
  ...
  bf16, and the official base quant documents no exclusions. This is narrower than the confound it
  replaced, and it is a difference between published artefacts rather than one introduced here.
